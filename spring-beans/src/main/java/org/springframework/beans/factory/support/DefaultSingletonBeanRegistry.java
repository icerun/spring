...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	...
	**/
	...
	/**
	  @解析：三级缓存，用来解决AOP的循环依赖问题
	 */
	private final Map<String, ObjectFactory<?>> singletonFactories = new HashMap<>(16);

	/** Cache of early singleton objects: bean name to bean instance. */
	/** 早期的单例对象的缓存：Bean名称到Bean实例. */
	/**
	 * @解析：这个就是传说中的二级缓存，保存的是半成品
	 */
	 ...
	 * 		第一步:先去单例缓存次中获取，看是否有创建好的对象,有直接返回对象.
	 * 		第二步:在单例缓存池singletonObjects中没有找到对象, 且singletonsCurrentlyInCreation 保存了当前正在创建的对象，那么可能标识
	 * 二级缓存中有对象 就会尝试从二级缓存中获取
	...
	protected Object getSingleton(String beanName, boolean allowEarlyReference) {
		...
		// 【翻译】快速检查现有实例，而无需完全单例锁定
		// 【注释】第一步：先去单例缓存池中获取，看有没有创建好的对象，有直接返回
		Object singletonObject = this.singletonObjects.get(beanName);
		// 第二步：如果从单例缓存池singletonObject没找到，但是singletonsCurrentlyInCreation里能找到（singletonsCurrentlyInCreation里保存了正在创建的对象）
		if (singletonObject == null && isSingletonCurrentlyInCreation(beanName)) {
			singletonObject = this.earlySingletonObjects.get(beanName);
			if (singletonObject == null && allowEarlyReference) {
				// 第三步，二级缓存拿不到，则去三级缓存获取
				synchronized (this.singletonObjects) {
	...
	public void registerDependentBean(String beanName, String dependentBeanName) {
		...
		synchronized (this.dependenciesForBeanMap) {
			// 添加dependendBeanName依赖于cannoicalName的映射关系到 存储 bean名到依赖于该bean名的bean名的Map中
