...
public abstract class AbstractXmlApplicationContext extends AbstractRefreshableConfigApplicationContext {
	...
	protected void loadBeanDefinitions(XmlBeanDefinitionReader reader) throws BeansException, IOException {
		// 获取资源的定位
		// 这里getConfigResources是一个空实现，真正实现是调用子类的资源定位的方法
		// 比如：ClasspatchXmlApplicationContext中进行了实现
		// 而FileSystemXmlApplicationContext没有使用该方法
		// 以Resource的方式获得配置文件的资源位置，（启动类参数启用）
		Resource[] configResources = getConfigResources();
		if (configResources != null) {
			// XML Bean读取器调用其父类AbstractBeanDefinitionReader读取定位的资源
			reader.loadBeanDefinitions(configResources);
		}
		// 如果子类获取的资源定位为空，则获取FileSystemXmlApplicationContext构造方法中setConfigLocation方法设置的资源
		// 以String的形式获得配置文件的位置
