...
/**
 * 【注释】解析<aop:config /> 标签的类。同样也是在AopNamespaceHandler中创建的类。
 *
 *
...
class ConfigBeanDefinitionParser implements BeanDefinitionParser {
	...
	/**
	 * 篇日志 支持aop:config标签创建的BeanDefinition的自动代理创建器
	...
	private void parseAdvisor(Element advisorElement, ParserContext parserContext) {
		...
		try {
			...
			// 解析point-cut属性并赋值到DefaulBeanFactoryPointcutAdvisor#pointcut内部属性
	...
	private void parseAspect(Element aspectElement, ParserContext parserContext) {
		...
		try {
			...
			for (int i = 0; i < nodeList.getLength(); i++) {
				...
				// 是否为advice: before/advice: after/advie:after-returning/advice:after-throwing/advice:around节点
				if (isAdviceNode(node, parserContext)) {
					// 校验aop:aspect必须有ref属性否则无法对切入点进行观察操作
	...
	private AbstractBeanDefinition parseAdvice(
		...
		try {
			...
			// 解析advice节点中的“method”属性，并包装为methodLocatingFactoryBean
